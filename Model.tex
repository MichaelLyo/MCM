...
\par For Problem 1.A, in order to lay the foundation of analyzing, we are required to create an energy profile for each of the four states. Meanwhile, the profiles should be created on the analysis of the given data.

\par For Problem 1.B, it adds a factor of consideration to the time dimension based on question A. Especially, the type of energy that needs to be analyzed becomes renewable energy, and the focus shifted from within a single state to between states. Explaining briefly the similarities and differences.

\par For problem 1.C. To determine the best performing profile among the REU profiles in each state in 2009, we need to inspect the comprehensive factors based on the calculation and comparison of previous model.

\par For Problem 1.D, in order to predict energy profile of usage of each state in 2025 and 2050, we need to base on the results of Problem A, B, C and the given data to establish regression equations or other models.

\subsubsection{Part \uppercase\expandafter{\romannumeral2}}

\par It is suggested in the Problem 2.A that, we should determine targets of the renewable energy usage for the four states in 2025 and 2050, which is based on the problems of Part \uppercase\expandafter{\romannumeral1}. 

\par As for Problem 2.B, we are supposed to give some suggestions to the governors, so that they will know that the actions they should take to meet the goals.
