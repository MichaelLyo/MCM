...
\par Factor Analysis is a multivariate statistical analysis method, which is based on the internal dependencies of the correlation matrix of the indicator. It attributes some of the variables that have complex relationship to a few irrelevant factors.\cite{2} We will use the method to deal with the problem A.

\par  By observing the given data, we can find that the MSNs are five-character codes, and the parts of the characters have generally have the meaning as follows.
...
Fifth character: describes a type of data (for example, P for data in physical unit, B for data in billion Btu)	\cite{3}
By simply analyzing the relationship between the fifth characters and the Units, we can find that 
